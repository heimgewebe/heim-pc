...
* **Readiness vor Setup:** Bei Audio-Transkription zuerst die zurückgegebene kanonische ASR-Route mit `doctor` prüfen. Nur wenn diese frische Prüfung eine fehlende lokale Runtime oder einen fehlenden Modellcache belegt, den kanonischen `setup`-Pfad einmalig verwenden. Kein per-Auftrag-`pip install`, keine per-Auftrag-venv und kein separater Modellcache. Der gemeinsame lokale Runtime-/Modellcache liegt unter `~/.local/cache/heimgewebe/asr/`; der Heim-PC-Locator verweist auf die kanonische Authority in `${HOME}/repos/asr`.
* Für Audio-Transkription keine Engine im Consumer pinnen und nicht automatisch zu Cloud-/Metered-Diensten eskalieren; die Engine- und Kostenentscheidung bleibt beim zur Ausführungszeit gelesenen ASR-Policy-Vertrag. Das kanonische Runbook `${HOME}/repos/asr/runbooks/asr-local-transcription.md` beschreibt Diagnose, Wiederverwendung und Verifikation; `runbooks/asr-local-transcription.md` ist nur die Host-Kompatibilitätsprojektion.
