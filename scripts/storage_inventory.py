...
def _validate_budget(name: str, value: Any) -> dict[str, int]:
    ...
    if (
        ...
        raise PolicyError(f"invalid budget for {name}")
    return {"warning": warning, "hard": hard}


def _validate_policy_path(raw: Any, *, owner: str) -> str:
    if not isinstance(raw, str) or not raw or "\x00" in raw:
        raise PolicyError(f"{owner} paths must be non-empty strings")
    if not (raw == "${HOME}" or raw.startswith("${HOME}/") or raw.startswith("/")):
        raise PolicyError(f"{owner} path must be absolute or HOME-relative")
    return raw
...
def load_policy(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(classes, dict) or not classes:
        raise PolicyError("classes must be a non-empty object")
    for class_name, class_spec in classes.items():
        if not isinstance(class_name, str) or not class_name:
            raise PolicyError("storage class names must be non-empty strings")
        if (
            not isinstance(class_spec, dict)
            or class_spec.get("automatic_cleanup") is not False
        ):
            raise PolicyError(
                f"storage class {class_name} requires automatic_cleanup=false"
            )

    thresholds = policy.get("filesystem_thresholds_percent")
    if not isinstance(thresholds, dict):
        raise PolicyError("filesystem_thresholds_percent must be an object")
    threshold_values = [
        thresholds.get("notice"),
    ...
    if (
        ...
        raise PolicyError(
            "filesystem thresholds must be ordered integers between 0 and 100"
        )

    _validate_budget(
        "global_temporary_budget_bytes",
        policy.get("global_temporary_budget_bytes"),
    )
    growth_thresholds = policy.get("growth_thresholds_bytes_24h")
    if growth_thresholds is not None:
        _validate_budget("growth_thresholds_bytes_24h", growth_thresholds)

    retention = policy.get("snapshot_retention")
    max_count = retention.get("max_count") if isinstance(retention, dict) else None
    if (
        not isinstance(max_count, int)
        or isinstance(max_count, bool)
        or max_count < 1
    ):
        raise PolicyError("snapshot_retention.max_count must be a positive integer")

    safety = policy.get("safety")
    if not isinstance(safety, dict):
        raise PolicyError("safety must be an object")
    follow_symlinks = safety.get("follow_symlinks", False)
    cross_filesystems = safety.get("cross_filesystems", False)
    unknown_owner_auto_cleanup = safety.get("unknown_owner_auto_cleanup", False)
    if follow_symlinks is not False:
        raise PolicyError("follow_symlinks must remain false")
    if not isinstance(cross_filesystems, bool):
        raise PolicyError("cross_filesystems must be boolean")
    if unknown_owner_auto_cleanup is not False:
        raise PolicyError("unknown_owner_auto_cleanup must remain false")
    does_not_establish = safety.get("does_not_establish")
    if (
        ...
        raise PolicyError("safety.does_not_establish must be a non-empty string list")
    ...
    for producer in producers:
        ...
        if (
            not isinstance(producer.get("cleanup_strategy"), str)
            or not producer["cleanup_strategy"]
        ):
            raise PolicyError(f"producer {producer_id} requires cleanup_strategy")
        raw_paths = producer.get("paths")
        if not isinstance(raw_paths, list) or not raw_paths:
            raise PolicyError(f"producer {producer_id} requires paths")
        for raw_path in raw_paths:
            validated_path = _validate_policy_path(raw_path, owner=producer_id)
            if validated_path in seen_paths:
                raise PolicyError(f"duplicate producer path: {validated_path}")
            seen_paths.add(validated_path)
        _validate_budget(
            f"producer {producer_id}",
            producer.get("budget_bytes"),
        )
    ...
    if discovery is not None:
        ...
        if (
            ...
            raise PolicyError(
                "unowned_discovery minimum_bytes must be a non-negative integer"
            )
        ...
        for root in roots:
            if not isinstance(root, dict):
                raise PolicyError("unowned discovery root must be an object")
            raw_path = _validate_policy_path(
                root.get("path"),
                owner="unowned discovery root",
            )
            if raw_path in discovery_paths:
                raise PolicyError(f"duplicate discovery root: {raw_path}")
            discovery_paths.add(raw_path)
            ...
            if (
                ...
                raise PolicyError(
                    "unowned discovery root requires non-empty name_globs"
                )
    return policy
...
def scan_path(path: Path, *, cross_filesystems: bool = False) -> ScanResult:
    ...
    if stat.S_ISLNK(root_mode) or stat.S_ISREG(root_mode):
        ...
        try:
            with os.scandir(current) as entries:
                for entry in entries:
                    ...
                    entry_mode = stat_result.st_mode
                ...
                for entry in entries:
                    try:
                        ...
                        errors.append({
                            ...
                            "error": f"discovery_entry_stat_failed:{exc.errno}",
                        })
                        continue
