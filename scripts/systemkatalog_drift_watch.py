...
ACTIVE_STATUSES = {"active", "waiting", "blocked", "in_progress"}
...
def _latest_candidate_status(payload: dict[str, Any], candidate_id: str) -> str | None:
    ...
    for item in records:
        ...
        if isinstance(record, dict) and record.get("candidate_id") == candidate_id:
            ...
            matching.append((event_id if isinstance(event_id, int) else -1, record.get("status")))
    if not matching:
        return None
...
def _ensure_bureau_candidate(bureau_root: Path, report_path: Path, report: dict[str, Any]) -> dict[str, Any]:
    ...
    latest = _latest_candidate_status(payload, CANDIDATE_ID)
    if latest in ACTIVE_STATUSES:
        return {"action": "deduplicated", "candidateId": CANDIDATE_ID, "status": latest}
    ...
    return {"action": "registered", "candidateId": CANDIDATE_ID, "eventId": receipt.get("event_id")}
