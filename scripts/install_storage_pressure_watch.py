#!/usr/bin/env python3
"""Install one commit-bound lightweight storage-pressure watch release."""
...
SERVICE_TEMPLATE = ROOT / "systemd/user/heim-pc-storage-pressure-watch.service.in"
TIMER_SOURCE = ROOT / "systemd/user/heim-pc-storage-pressure-watch.timer"
SOURCES = {
    "scripts/storage_pressure_watch.py": ROOT / "scripts/storage_pressure_watch.py",
    "config/storage-pressure.v1.json": ROOT / "config/storage-pressure.v1.json",
}
UNIT_NAME = "heim-pc-storage-pressure-watch"
...
def verify_unit_files(service_path: Path, timer_path: Path) -> dict[str, Any]:
    ...
    diagnostics = [
        line for line in completed.stderr.splitlines() if any(path in line for path in targets)
    ]
    if diagnostics:
        raise InstallError(
            "unit verification reported target diagnostics: " + " | ".join(diagnostics[:10])
        )
    if completed.returncode == 0:
...
def install(
    *,
    ...
    service_template = repository_blob(
        ROOT,
        head=head,
        relative_path="systemd/user/heim-pc-storage-pressure-watch.service.in",
    )
    timer_data = repository_blob(
        ROOT,
        head=head,
        relative_path="systemd/user/heim-pc-storage-pressure-watch.timer",
    )
    ...
    if apply:
        state_directory = home / ".local/state/heim-pc/storage-pressure-watch"
        ...
        if (
            ...
            raise InstallError(f"runtime directory is unsafe: {state_directory}")
        ...
        verification = verify_unit_files(service_target, timer_target)
        ...
        for unit in (f"{UNIT_NAME}.service", f"{UNIT_NAME}.timer"):
            ...
            if state != "loaded":
                raise InstallError(f"systemd unit did not load: {unit}={state!r}")
        if enable:
    ...
    receipt = {
        "schema_version": 1,
        "kind": "heim_pc_storage_pressure_watch_install_receipt",
    ...
    if apply:
        receipt_path = (
            home
            / ".local/state/heim-pc/storage-pressure-watch/install-receipts"
            / f"{head}.json"
        )
        atomic_install(
            receipt_path,
...
def main() -> int:
    ...
    parser.add_argument(
        "--release-root",
        type=Path,
        default=Path.home() / ".local/lib/heim-pc/storage-pressure-watch/releases",
    )
    ...
    try:
        ...
        print(
            json.dumps(
                {"kind": "heim_pc_storage_pressure_watch_install_error", "error": str(exc)},
                sort_keys=True,
            ),
            file=sys.stderr,
        )
