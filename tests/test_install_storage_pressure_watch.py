...
SPEC = importlib.util.spec_from_file_location(
    "install_storage_pressure_watch", ROOT / "scripts/install_storage_pressure_watch.py"
)
...
class InstallStoragePressureWatchTests(unittest.TestCase):
    def test_install_renders_commit_bound_release_and_units(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            blobs["systemd/user/heim-pc-storage-pressure-watch.service.in"] = (
                installer.SERVICE_TEMPLATE.read_bytes()
            )
            blobs["systemd/user/heim-pc-storage-pressure-watch.timer"] = (
                installer.TIMER_SOURCE.read_bytes()
            )
            ...
            with (
                ...
                patch.object(
                    installer,
                    "repository_blob",
                    side_effect=lambda _root, *, head, relative_path: blobs[relative_path],
                ),
            ...
            self.assertTrue((release / "scripts/storage_pressure_watch.py").is_file())
            self.assertTrue((release / "config/storage-pressure.v1.json").is_file())
            service = (
                home / ".config/systemd/user/heim-pc-storage-pressure-watch.service"
            ).read_text()
            ...
            timer = (
                home / ".config/systemd/user/heim-pc-storage-pressure-watch.timer"
            ).read_text()
