...
class StorageInventoryTests(unittest.TestCase):
    ...
    def test_scan_special_file_does_not_attempt_directory_traversal(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            fifo = Path(directory) / "fifo"
            os.mkfifo(fifo)

            result = scan_path(fifo)
    ...
    def test_atomic_write_fsyncs_file_and_parent_directory(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "scripts.storage_inventory.os.fsync", wraps=original_fsync
            ) as fsync_mock:
                _atomic_write_json(output, {"ok": True})

            self.assertGreaterEqual(fsync_mock.call_count, 2)
            self.assertEqual(json.loads(output.read_text()), {"ok": True})

    def test_policy_rejects_automatic_cleanup_class(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            tmp_path = Path(directory)
            policy_path = make_policy(tmp_path / "policy.json", tmp_path / "producer")
            data = json.loads(policy_path.read_text())
            data["classes"]["temporary_workspace"]["automatic_cleanup"] = True
            policy_path.write_text(json.dumps(data))

            with self.assertRaisesRegex(PolicyError, "automatic_cleanup=false"):
                load_policy(policy_path)

    def test_policy_rejects_unknown_owner_auto_cleanup(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            tmp_path = Path(directory)
            policy_path = make_policy(tmp_path / "policy.json", tmp_path / "producer")
            data = json.loads(policy_path.read_text())
            data["safety"]["unknown_owner_auto_cleanup"] = True
            policy_path.write_text(json.dumps(data))
