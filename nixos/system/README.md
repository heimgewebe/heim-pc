...
Managed activation v1 supports only `test` and `next-boot`. Persistent-promotion v2 has a validation-only contract that binds the exact build/source/closure, the externally bound prior persistent-state digest, a typed post-effect promotion observation, and a fresh exact current observation for rollback. The typed observations carry bounded freshness (`observed_at` plus at most 300 seconds), and the current observation must be strictly newer than the promotion observation; runtime execution and observation provenance/authenticity remain separate successor proof work.
...
The production path is deliberately split into **local prepare -> independent rebuild/attest -> plan -> apply** and remains separate from the disposable Freecom/T017 rehearsal authority. `../../scripts/nixos_production_prepare.py` routes automated work through the Nix-specific `../../scripts/managed_build.py` plan/run profile, accepts only a clean exact Git revision, bundles that revision, evaluates and builds `heim-pc-storage-target` in the pinned Nix Docker image, and retains the resulting `/nix` closure in a revision-named Docker volume. Managed Build holds an exclusive lifecycle lock plus persistent active fence, monitors the store against a stop threshold below the hard budget while the worker runs, treats every incomplete store scan as fail-closed, and on budget/scan/timeout termination kills and verifies the full worker process group plus every per-run-labeled Docker container before releasing the lifecycle. A successful local run emits a create-only install-artifact JSON and success receipt, but that local receipt alone is not production authority. For `merged-main`, `.github/workflows/nixos-production-build-attest.yml` receives the exact small local candidate **and its exact local managed-build receipt bytes**, validates that candidate/receipt pair against the current managed policy, independently rebuilds exactly the same `refs/heads/main` commit through the same Managed-Nix production profile on a GitHub-hosted runner, validates its independent success receipt, and compares the security-relevant build identity: source revision, exact system closure, closure-manifest digest/count, deterministic volume name, pinned Nix image, profile and authority. The signed predicate binds both the candidate receipt byte digest and its semantic `managed_receipt_sha256`, so Apply does not trust a same-user sidecar by ownership alone. `source_bundle_sha256` is deliberately excluded from cross-host equality because Git bundle packing is not required to be byte-deterministic and Apply never uses that digest as authority. Only after the independent rebuild matches does GitHub Artifact Attestations/Sigstore attest the exact local candidate bytes with a signed custom predicate binding the independent artifact digest, independent success-receipt/managed-receipt digests, managed-policy digest and closure-semantic digest. The independent remote `/nix` closure is not transferred. The local Managed-Nix closure remains only a staging source: its success receipt now also binds the exact canonical managed `store_root`. Immediately before a production Apply, root requires `/nix` to be absent and first freezes the already pinned `nixos/nix:2.35.2` verifier image into a root-owned mode-0400 immutable archive under `/var/lib/heim-pc/nixos-production-verifiers/`. The archive is parsed without extraction: its Docker config bytes must hash to the exact pinned image ID and its tag/layer metadata must be structurally valid. Root then records the Docker service/socket state plus the exact full IDs of every container running before Apply, stops Docker, proves the service/socket are inactive and that no `containerd-shim-runc-v2 -namespace moby` survives, and rechecks the same immutable archive digest. Only after that trust handoff is the complete managed `/nix` tree (`store` plus Nix state) sealed into a separate root-private, mode-0400 immutable SquashFS image under `/var/lib/heim-pc/nixos-production-seals/`, attached through a read-only loop device and mounted at canonical `/nix`. The archive is imported into a fresh plan-bound namespace of the host containerd through its root-only socket; a read-only container rootfs receives the sealed `/nix` only as `/subject/nix` via `rbind:ro`, has no CNI or host network, and independently runs `path-info` plus `nix store verify` against the sealed subject store. Namespace and verifier archive must be removed and proven absent before any storage mutation. Only this independently verified sealed canonical `/nix` is an installation source. Docker stays quiesced through the last storage command. Only after seal teardown is the pre-Apply service/socket state restored; every container ID that was running before Apply is then required to be running again, with missing IDs explicitly restarted and read back instead of relying on restart policies. A branch-head artifact remains `proof-only`; it may be planned without a production attestation but Apply rejects it unconditionally.

`../../scripts/nixos_production_install.py` consumes the exact local artifact plus a mandatory local `--identity-contract`. For a `merged-main` artifact it also derives the sibling `.managed-build-attestation.json` bundle and verifies the exact artifact bytes with `/usr/bin/gh attestation verify`. Verification pins repository `heimgewebe/heim-pc`, signer workflow `.github/workflows/nixos-production-build-attest.yml`, the exact signer/source commit, `refs/heads/main`, and rejects self-hosted runners; Apply repeats the same verification immediately before destructive effects. The private storage identity remains a separate mode-0600 local contract bound to source revision and public-contract digest. Kernel names remain observation-only. Preliminary closure verification still uses the pinned-image Nix binary against the local prepared store. The final mutation authority is stricter: after Docker quiesce, the root-owned SquashFS is mounted read-only at `/nix`; cryptographic closure verification comes from the separately pinned Nix image imported into the root-only host-containerd namespace, not from an executable inside the closure being verified. After that independent verification, Apply keeps checking the kernel mount/loop/immutable/path-containment properties before the race-closing hardware/attestation gate and before every storage effect. A valid remote attestation therefore cannot substitute an absent, changed, or unsealed local closure.

For `merged-main`, planning additionally requires a private `--pre-cutover-readiness` bundle. The bundle never grants production authority: it must bind the exact source revision, the byte digests of `recovery-contract-v1.json` and `nix-lifecycle-contract-v1.json`, the exact required recovery-evidence ID set, each private receipt path and receipt digest, and the reviewed freshness window while declaring `production_effects_authorized=false`. Bundle and receipts are opened fail-closed through no-follow descriptor-relative path traversal, must be single-link bounded regular files owned by the invoking authority with mode `0600` or stricter, and their file identities are bound into the private plan. Immediately before the first planned storage mutation, after the final Docker/seal read-only guards, Apply re-verifies the exact Git source revision, reopens the bundle and all receipts, rehashes both contracts and re-evaluates freshness. Any source, bundle, receipt, contract or file-identity drift fails with `mutation_attempted=false` before a storage command can run. Initial cutover requires the lifecycle contract but not a running NixOS live-lifecycle audit; automatic GC remains disabled, and any later automatic-GC enablement requires a fresh live audit that proves the declared last-known-good store path is backed by an enumerated Nix GC root, plus separate reviewed enablement.

Before any effect, the planner requires the target to remain blank and unmounted, checks hidden signatures with read-only `wipefs`, proves `/` and `/boot/efi` still resolve to the protected WD, verifies all declared WD by-id aliases, captures the live GPT disk GUID plus partition offsets/type GUIDs/names/attributes into the preimage fingerprint, rejects target/protected alias collisions, verifies the exact artifact/source revision and scratch state, and binds the plan to the private identity digest. The default CLI output contains only a redacted plan summary; a full private plan requires explicit `--write-plan` and is create-only mode 0600. Apply additionally requires root, a `merged-main` artifact, the exact plan confirmation, and interactive credentials. GPT creation uses private plan-bound PARTUUIDs; formatting and LUKS address only target-derived stable by-id partition paths. Public PARTLABELs are never boot or mount authority. Apply gives the LUKS2 header the private encrypted-partition UUID, stages all three private PARTUUIDs create-only under encrypted `/persist`, and binds NixOS systemd-boot entries locally with `rd.luks.name=<private-uuid>=heimpc-nixos-crypt`. The reproducible closure contains only generic code: after root unlock it mounts `/boot` and `/recovery` through the private `/persist` identity and `/dev/disk/by-partuuid`, and its systemd-boot hook rebinds future regenerated entries without embedding concrete private IDs in the Store. The critical Apply phase deliberately removes Docker from the executor path rather than relying on container permissions: after Docker has been proven quiescent and the root-only containerd verifier has independently accepted the seal, `mkfs.btrfs`, `btrfs` and `nixos-install` execute directly by exact absolute path from the immutable attested system closure under the read-only `/nix/store`, with a bounded `PATH`. This trades a short Docker outage during the later real cutover for eliminating the user-controlled Docker/root bypass and the verify-then-swap race. Any Docker reactivation, surviving Moby shim, seal teardown uncertainty, or Docker-state restore uncertainty fails closed; post-mutation cleanup/restore failures have dedicated public alarms. The installed profile, Seagate ESP, encrypted `@persist`, EFI/NVRAM state, complete fallback GPT fingerprint and closed mapper are all verified after the run.
