...
`../../scripts/nixos_production_install.py` consumes that exact artifact. Its default mode is effect-free observation plus plan compilation; Pop!_OS itself does not need a host Nix installation. The target mutation authority is exactly `/dev/disk/by-id/nvme-Seagate_ZP4000GP304001_7VS01DX7` (model `Seagate ZP4000GP304001`, serial `7VS01DX7`, WWN `eui.6479a7716f000a39`, size `4000787030016`). Kernel names such as `nvme0n1` are observation-only and are rejected as selection authority. Btrfs tooling and `nixos-install` execute offline from the exact prepared system closure in the pinned image/volume, not from ambient host binaries or a mutable channel.

Before any effect, the planner requires the Seagate to remain blank and unmounted, checks hidden signatures with read-only `wipefs`, proves `/` and `/boot/efi` still resolve to the four-partition WD fallback fingerprint including filesystem UUID/type, rejects target/protected alias collisions, verifies the exact artifact/source revision and scratch state, and binds the reviewable plan to those facts. Apply additionally requires root, a private yescrypt hash file, two matching interactive LUKS passphrase entries and the exact `APPLY-NIXOS-PRODUCTION:<plan_sha256>` confirmation; the same hardware gates are re-read immediately before the first destructive command. GPT creation uses the contract's fixed partition GUIDs; formatting and LUKS use only `/dev/disk/by-partuuid/...`. The installed profile must resolve to the exact prepared closure, systemd-boot must exist on the Seagate ESP, `/persist` must be the encrypted `@persist` subvolume, and first-boot credentials are staged only there. EFI/NVRAM state and the WD identity/partition/filesystem fingerprint are hashed or re-read before and after the run and must remain unchanged; teardown must leave the LUKS mapper closed. Neither plaintext nor reusable credentials enter Git, argv, the Nix store or receipts.
