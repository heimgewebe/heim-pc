...
Managed activation v1 supports only `test` and `next-boot`. Persistent-promotion v2 has a validation-only contract that binds the exact build/source/closure plus typed pre- and post-effect persistent-state observations. Observations carry bounded freshness (`observed_at` plus at most 300 seconds), and rollback requires an exact externally bound current observation that is fresh and strictly newer than the promotion observation; runtime execution and observation provenance/authenticity remain separate successor proof work.
