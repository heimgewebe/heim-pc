...
The production path is deliberately split into **prepare -> plan -> apply** and remains separate from the disposable Freecom/T017 rehearsal authority. `../../scripts/nixos_production_prepare.py` routes automated work through the Nix-specific `../../scripts/managed_build.py` plan/run profile, accepts only a clean exact Git revision, bundles that revision, evaluates and builds `heim-pc-storage-target` in the pinned Nix Docker image, and retains the resulting `/nix` closure in a revision-named Docker volume. Managed Build holds an exclusive lifecycle lock plus persistent active fence, monitors the store against a stop threshold below the hard budget while the worker runs, and on budget/timeout termination kills the worker process group and verifies removal of every per-run-labeled Docker container before releasing the lifecycle. Only a successful clean run emits both the private crash-safe create-only install-artifact JSON and a create-only success receipt bound to the exact managed policy, artifact file/JSON digests, source revision, closure manifest and Docker volume. Plan and Apply require that receipt and re-read its artifact/policy bindings before storage mutation. A branch-head artifact is technically marked `proof-only`; `merged-main` can only be prepared after a fresh `origin/main` fetch proves the exact source revision, and Apply rejects `proof-only` artifacts unconditionally.
