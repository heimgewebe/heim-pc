...
Der Nix-Pfad bindet Repository/Revision/Lock-/Produktionsvertrag an die Managed-Build-Identität, legt den revision-bound `/nix`-Store unter einen Managed-Cache-Pfad und schützt den Lauf mit exklusivem Lifecycle-Lock plus persistentem Active-Fence. Der Store wird während des Builds gegen eine konservative Stop-Schwelle unterhalb des Hard-Limits überwacht; ein Budget- oder Runtime-Trigger beendet die Worker-Prozessgruppe und lässt den Lauf erst nach verifizierter Entfernung aller exakt per Run gelabelten Docker-Container konvergieren. Nur ein erfolgreicher, cleanup-verifizierter Lauf publiziert neben dem Artifact ein create-only Success-Receipt, das Source-Revision, Managed-Policy, Artifact-Datei/JSON, System-Closure, Closure-Manifest und Docker-Volume bindet. Plan und Apply konsumieren dieses Receipt erneut und prüfen es unmittelbar vor der ersten Storage-Mutation. `managed_nix.py` bleibt dabei ausdrücklich validation-only und wird nicht zum Executor erweitert.
