...
Öffentliche `HEIMPC_NIXOS_*`-PARTLABELs beschreiben ausschließlich Topologie und dienen nach der Partitionierung als zusätzliche Bindungsbeweise; sie sind **keine Runtime-Boot- oder Mount-Autorität**. Exakte GPT-PARTUUIDs sowie die Ziel-/Fallback-Identitäten liegen ausschließlich im lokalen, revisions- und Public-Contract-Digest-gebundenen Private-Identity-Contract. Beim Apply erhält der LUKS2-Header lokal dieselbe private UUID wie die verschlüsselte Zielpartition. Die lokale Installation schreibt die drei privaten PARTUUIDs create-only und mode 0600 nach verschlüsseltem `/persist`; systemd-boot erhält ausschließlich lokal den Kernelparameter `rd.luks.name=<private-uuid>=heimpc-nixos-crypt`. Ein generischer, reproduzierbarer NixOS-Dienst liest nach dem Root-Unlock die private `/persist`-Identität und mountet ESP sowie Recovery über `/dev/disk/by-partuuid/...`; sein Bootloader-Hook bindet auch spätere neu erzeugte NixOS-Einträge erneut an dieselbe private LUKS-Identität. Konkrete private UUIDs gelangen damit weder in den öffentlichen Contract noch in das unabhängig nachgebaute Nix-Closure. Destruktive Operationen wählen das Ziel weiterhin ausschließlich über die dort gebundene exakte `/dev/disk/by-id/...`-Identität plus Modell, Seriennummer, Größe und WWN. Kernel-Namen wie `/dev/nvme0n1` oder `/dev/nvme1n1` sind niemals Autorität.
...
Der Nix-Pfad bindet Repository/Revision/Lock-/Produktionsvertrag an die Managed-Build-Identität, legt den revision-bound `/nix`-Store unter einen Managed-Cache-Pfad und schützt den Lauf mit exklusivem Lifecycle-Lock plus persistentem Active-Fence. Der Store wird während des Builds gegen eine konservative Stop-Schwelle unterhalb des Hard-Limits überwacht; unvollständige Store-Scans sind selbst ein fail-closed Fehler. Budget-, Scan- oder Runtime-Abbruch beendet die gesamte Worker-Prozessgruppe und lässt den Lauf erst nach verifiziertem Gruppenstillstand und verifizierter Entfernung aller exakt per Run gelabelten Docker-Container konvergieren. Nur ein erfolgreicher Lauf publiziert neben dem Artifact ein create-only Success-Receipt. Dieses lokale Receipt bleibt Integritäts- und Lifecycle-Evidence, ist allein aber keine Produktionsautorität. Nach Merge erzeugt Heim-PC den verwendbaren `merged-main`-Kandidaten weiterhin lokal, damit die attestierte System-Closure im lokalen revision-bound Docker-Volume für Install und Apply vorhanden bleibt. Der GitHub-hosted Workflow `.github/workflows/nixos-production-build-attest.yml` erhält anschließend die exakten Kandidatenbytes, baut denselben finalen Main-Commit unabhängig über denselben Managed-Nix-Pfad nach, validiert das unabhängige Success-Receipt und fordert Gleichheit der sicherheitsrelevanten Build-Identität einschließlich Source-Revision, System-Closure, Closure-Manifest, Image und Profil. Lediglich `source_bundle_sha256` ist von der Cross-Host-Gleichheit ausgenommen, weil Git-Bundle-Packing nicht byte-deterministisch sein muss und dieser Digest keine Apply-Autorität besitzt. Erst nach erfolgreichem unabhängigen Match attestiert GitHub Artifact Attestations/Sigstore die exakten lokalen Kandidatenbytes zusammen mit einem signierten Custom-Predicate, das den unabhängigen Artifact-Digest, unabhängigen Success-Receipt-/Managed-Receipt-Digest, Managed-Policy-Digest und den Closure-Semantik-Digest bindet. Plan und Apply verifizieren diese Attestation mit `gh attestation verify` gegen `heimgewebe/heim-pc`, den exakten Signer-Workflow, denselben Workflow- und Source-Commit, `refs/heads/main` und `--deny-self-hosted-runners`. Das lokale Success-Receipt bindet zusätzlich den exakten kanonischen Managed-Store-Root. Die unabhängige Remote-Closure wird weder übertragen noch als Install-Store verwendet. Unmittelbar vor einem produktiven Apply verlangt Root ein freies `/nix` und friert zunächst das bereits auf Image-ID, Tag und Repo-Digest geprüfte Nix-Verifier-Image als root-owned, mode-0400 und immutable markiertes Docker-Archiv unter `/var/lib/heim-pc/nixos-production-verifiers/` ein. Die Config-Datei im Archiv muss selbst auf die gepinnte Image-ID hashen; Manifest-, Tag- und Layerstruktur werden ohne Extraktion geprüft. Danach speichert Root den Docker-Service-/Socket-Zustand sowie die exakten vollständigen IDs aller vor Apply laufenden Container, stoppt Docker und beweist zusätzlich die Abwesenheit sämtlicher Moby-`containerd-shim-runc-v2`-Prozesse. Das Verifier-Archiv wird nach diesem Quiesce erneut gegen denselben Digest geprüft. Erst dann wird der komplette lokale Managed-Nix-Root (`store` plus Nix-State) in ein separates root-privates, mode-0400 und immutable markiertes SquashFS unter `/var/lib/heim-pc/nixos-production-seals/` überführt und per read-only Loop an `/nix` gemountet. Für die kryptografische Closure-Prüfung wird das eingefrorene Verifier-Archiv in einen frischen, plan-gebundenen Namespace des hostseitigen Containerd über dessen `root:root`-Socket importiert; der Verifier läuft mit read-only Rootfs, erhält `/nix` ausschließlich als `/subject/nix` via `rbind:ro`, nutzt weder CNI noch Host-Network und führt `path-info` plus `nix store verify` gegen den versiegelten Subject-Store aus. Namespace und Archiv müssen vor der ersten Storage-Mutation vollständig entfernt sein. `mkfs.btrfs`, `btrfs` und `nixos-install` laufen anschließend im kritischen Abschnitt direkt per exaktem absolutem Pfad aus der versiegelten Closure; Docker bleibt bis nach der letzten Storage-Operation quieszent. Damit ist das benutzerkontrollierte Managed-Store-Verzeichnis nur Quelle für die Seal-Erzeugung, nicht mehr Ausführungsautorität. Unsicherer Seal-Teardown oder unsichere Docker-Wiederherstellung bleiben fail-closed; nach dem Seal-Teardown wird neben Service/Socket jede zuvor laufende Container-ID explizit wieder als laufend verlangt und bei Bedarf gezielt gestartet, statt Restart-Policies als Wiederherstellungsautorität zu behandeln. Nach Mutationsbeginn lösen Fehler dabei eigene Post-Mutation-Alarme aus. Branch-/Proof-Artefakte benötigen diese Produktionsattestation nicht, können aber weiterhin niemals Apply autorisieren. `managed_nix.py` bleibt validation-only.
...
Der produktive Pfad hat drei getrennte Stufen. Erstens routet `scripts/nixos_production_prepare.py` den automatisierten Build über `scripts/managed_build.py plan` und `run`, baut aus einem **cleanen exakten Git-Commit** im gepinnten Nix-Docker-Image den exakten `heim-pc-storage-target`-Closure in einem revisionsgebundenen `/nix`-Volume und publiziert dessen privates, crash-sicher create-only Install-Artefakt. Diese Stufe kennt keine Blockgeräte. Ein Artefakt eines PR-/Branch-Heads trägt technisch die Authority `proof-only`; `merged-main` darf nur nach frischem Fetch erzeugt werden, wenn der exakte Source-Commit dem beobachteten `origin/main` entspricht. Apply lehnt `proof-only` immer ab. Zweitens liest `scripts/nixos_production_install.py` Hardware und Artefakt effect-frei ein und kompiliert einen plan-hash-gebundenen Plan; stdout enthält standardmäßig nur eine redigierte Plan-Zusammenfassung, ein vollständiger Plan wird nur explizit create-only mit Mode 0600 geschrieben. Drittens darf Apply erst nach exakter Bestätigung, erneutem Live-Gate und interaktiver Secret-Zufuhr mutieren.

Pop!_OS benötigt dafür kein vorinstalliertes hostseitiges Nix. Der vorbereitete Store wird zunächst mit dem Nix-Binary aus dem gepinnten Image als separater read-only Local Store geprüft. Im produktiven Apply wird zunächst das gepinnte Nix-Image in ein root-owned unveränderliches Verifier-Archiv überführt. Danach wird Docker vollständig quiesziert, der komplette Managed-Nix-Root als root-owned SquashFS an das zuvor nicht vorhandene kanonische `/nix` versiegelt und von einem separaten Root-only-Containerd-Verifier aus dem eingefrorenen gepinnten Image geprüft. Der Seal verifiziert sich also nicht selbst. Nach erfolgreichem Verify werden Containerd-Namespace und Archiv entfernt; anschließend laufen `nixos-install`, `mkfs.btrfs` und `btrfs` offline per exaktem absolutem Pfad direkt aus dieser unveränderlichen Closure. Kein Docker-Container und kein Containerd-Verifier liegt mehr im mutierenden Executor-Pfad. Schreibende Storage-Befehle adressieren ausschließlich die exakte private Seagate-by-id beziehungsweise daraus abgeleitete stabile by-id-Partitionaliases; private PARTUUID- und öffentliche PARTLABEL-Aliase dienen nur als zusätzliche Bindungsbeweise nach der Partitionierung. Vor der ersten Mutation und nach dem Lauf muss der WD-Fingerprint inklusive Root/ESP-Zuordnung, PARTUUIDs sowie Filesystem-Typen/-UUIDs identisch sein. Die Seagate erhält ihre eigene GPT, ESP, Recovery-Fläche, LUKS2- und Btrfs-Struktur sowie systemd-boot; der EFI/NVRAM-Digest muss unverändert bleiben. Vor dem ersten Boot werden die source- und Hash-gebundenen Firstboot-Credentials ausschließlich in das verifizierte verschlüsselte `@persist` gestaged; nach Teardown muss der Mapper geschlossen sein.
